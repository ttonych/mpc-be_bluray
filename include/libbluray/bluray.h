...
/** MPC-BE local extension, not part of upstream libbluray 1.5.0.
 * After a click misses all active buttons, try a single invisible HDMV
 * page-return command whose destination has a button at (x,y).
 * Returns 1 if the command was queued, 0 if no unambiguous route exists.
 * Caller must run bd_read_ext with len=0, then call bd_mouse_select again
 * before activating anything. Does not advance the media stream.
 */
BD_PUBLIC int bd_mouse_select_page(BLURAY *bd, int64_t pts, uint16_t x, uint16_t y);
